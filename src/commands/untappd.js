...
function formatBeerInfoSlackMessage(source, query, beerInfos) {
  ...
  for (let beerInfo of beerInfos) {
    ...
    let attachment = {
      ...
      text: `${ratingString} (${beerInfo.rating_count} ratings)\n_${beerInfo.beer_style} — ${
        beerInfo.beer_abv
      }% ABV — ${beerInfo.beer_ibu || 0} IBU_`
    };
    if (beerInfo.brewery)
...
const handler = async function(payload, res) {
  try {
    ...
    const beerIds = await Promise.all(
      payload.text.split(",").map(x => util.searchForBeerId(x.trim()))
    ).catch(util.onErrorRethrow);
    const beerInfos = await Promise.all(beerIds.map(x => util.getBeerInfo(x))).catch(
      util.onErrorRethrow
    );
